...
const dailySummaryAdvice = async (req, res) => {
    try {
        if (!req.userId) {
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a practical personal finance coach. Return ONLY valid JSON with keys: briefSummary (string), doList (array of max 4 strings), avoidList (array of max 4 strings). Keep advice specific to the given day.",
                    },
                    {
                        role: "user",
        ...
        const advice = {
            ...
            expense,
            balance,
            briefSummary: typeof parsed.briefSummary === "string" && parsed.briefSummary.trim()
